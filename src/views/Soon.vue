<template>
    <div class="soon">
        ...
        <Paragraph styled-like="description">Графики формируются на основании статистики, полученной из Github API
        </Paragraph>
    </div>
</template>
            ...
            github.forEach((item, i) => {
                this.commits.push(...item.days.map((d, j) => {
                    if ((i * 7 + j) * 86400000 + Date.parse(date) <= Date.now()) {
                        ...
                        return {
                            amount: d,
                            date: `${temp.getDate() >= 10 ? temp.getDate() : '0' + temp.getDate()}.${(temp.getMonth() + 1) >= 10 ? (temp.getMonth() + 1) : '0' + (temp.getMonth() + 1)}`
                        }
                    }
                }))
            })
